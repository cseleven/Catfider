...
async function getdataPerDate( shelter_id, result ) {
  ...
  const { data, count } = await supabase.from('adopt').select('adopt_id, cat_id, cat_profile(cat_name, breed, sex, create_date), user_id, user_profile(email), adopt_date,  shelter_id, shelter_profile(shelter_name)', { count: 'exact' }).eq('shelter_id', shelter_id).gte('adopt_date', currentDate)
  result = {
    data,
    count
  }
...
async function getdataPerMonth( shelter_id, result ) {
  ...
  //query data
  const { data, count } = await supabase.from('adopt').select('adopt_id, cat_id, cat_profile(cat_name, breed, sex, create_date), user_id, user_profile(email), adopt_date,  shelter_id, shelter_profile(shelter_name)', { count: 'exact'}).eq('shelter_id', shelter_id).gte( 'adopt_date', currentMonth)
  result = {
    data,
    count
  }
...
async function getdataPerYear(shelter_id, result) {
  ...
  //query data
  const { data, count } = await supabase.from('adopt').select('adopt_id, cat_id, cat_profile(cat_name, breed, sex, create_date), user_id, user_profile(email), adopt_date,  shelter_id, shelter_profile(shelter_name)', { count: 'exact' }).eq('shelter_id', shelter_id).gte('adopt_date', currentYear)
  result = {
    data,
    count
  }
